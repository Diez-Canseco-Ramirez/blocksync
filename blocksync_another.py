...
def sync(srcdev, dsthost, dstdev=None, blocksize=1024 * 1024):
    ...
    print
    "Block size is %0.1f MB" % (float(blocksize) / (1024 * 1024))
    ...
    if a != dstdev:
        print
        "Dest device (%s) doesn't match with the remote host (%s)!" % (dstdev, a)
        sys.exit(1)
    if int(b) != blocksize:
        print
        "Source block size (%d) doesn't match with the remote host (%d)!" % (blocksize, int(b))
        sys.exit(1)
    ...
    try:
        ...
        print
        "Error accessing source device! %s" % e
        sys.exit(1)
    ...
    if p.returncode is not None:
        print
        "Error accessing device on remote host!"
        sys.exit(1)
    remote_size = int(line)
    if size != remote_size:
        print
        "Source device size (%d) doesn't match remote device size (%d)!" % (size, remote_size)
        sys.exit(1)
    ...
    for i, l_block in enumerate(getblocks(f, blocksize)):
        ...
        if t1 - t_last > 1 or (same_blocks + diff_blocks) >= size_blocks:
            ...
            print
            "\rsame: %d, diff: %d, %d/%d, %5.1f MB/s" % (
            same_blocks, diff_blocks, same_blocks + diff_blocks, size_blocks, rate),
            t_last = t1
...
if __name__ == "__main__":
    ...
    parser.add_option("-b", "--blocksize", dest="blocksize", action="store", type="int", help="block size (bytes)",
                      default=1024 * 1024)
